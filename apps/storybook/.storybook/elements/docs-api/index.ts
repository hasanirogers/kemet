...
import CEM from 'kemet-ui/custom-elements.json';
  ...
  getData() {
    CEM.modules.find((module) => {
      ...
      if (info) {
        ...
        this.members = info.members.filter((member: any) => member.privacy === 'public') ?? null;
      ...
      <kemet-accordion>
        <kemet-accordion-panel
          ...
          @kemet-opened=${(event: CustomEvent) => {
            const icon = (event.target as HTMLElement).querySelector('kemet-icon-bootstrap') as any;
